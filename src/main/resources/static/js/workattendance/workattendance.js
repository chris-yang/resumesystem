...
function applyModify(title,url,projectId,applyCount,totalApplyCount,table,layer,w,h) {
    ...
    if(getSubStatus(data,'A').length >0) {
        var noSubmitData = getSubStatusContent(data, 'A');
        layer.msg("申请修改中的数据【" + noSubmitData + "】不可重复修改", {icon: 5});
        return;
    }
...
//根据提交的状态S N获得对应的数组列表
function getSubStatusContent(data,subStatusValue) {
    var idSubStatus=[];
    var j=0;
    for(var i=0;i<data.length;i++) {
        //提交状态
        if(checkSubStatus(data[i].subStatus,subStatusValue)){
            idSubStatus[j++]=data[i].employeeName;
        }
    }
    return idSubStatus;
}

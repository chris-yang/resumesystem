...
import com.beyondsoft.mapper.SysDeptMapper;
import com.beyondsoft.mapper.SysRoleMapper;
import com.beyondsoft.service.SysDeptRoleService;
...
//        SysRole sysRole = sysRoleMapper.getSysRoleByUserId(map);
    ...
    public SysDeptVo getRootSysDeptVoById(String rootCode) {
        ...
        if(sysDeptVo!=null) {
            ...
            sysDeptVo = new SysDeptVo();
            sysDeptVo.setParentContent("请添加部门");
        }
     ...
     */
    @Override
    public Integer getSortValue(Integer pid) {
        Integer sort = this.baseMapper.getSortValue(pid);
        if(sort!=null && sort>0) {
            sort=sort+1;
        } else {
            sort=1;
